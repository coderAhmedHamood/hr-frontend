...
| من يستقبل إشعارات المخازن؟ | **مستخدمون** (`user_id`) — وليس موظفو HR |
...
| `inv.notifications.read` | صندوق الوارد + عدد غير المقروء |
...
| نوع الحدث | audienceKind | من يستلم |
|---|---|---|
| حركة مستودع مرتبطة بفرع | `branch` | مستخدمون نشطون في `user_companies` + `user_branches` لذلك الفرع |
| تنبيهات مخزون / بدون فرع | `company` | كل المستخدمين النشطين في الشركة |

- **لا** يُستخدم `employee` أو `department` — تطبيق مخازن بدون HR.
- المستخدم يجب أن يكون `is_active` في `user_companies` (وفي `user_branches` عند audience=branch).
...
الإرسال **best-effort** — فشل الإشعار لا يوقف العملية الأساسية.
...
*آخر تحديث: أغسطس 2026 — إشعارات المخازن مفعّلة ومستقلة عن HR.*
