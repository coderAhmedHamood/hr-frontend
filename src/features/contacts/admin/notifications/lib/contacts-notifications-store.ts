...
export interface ContactsNotificationRecord {
  id: string;
...
function mapApi(row: InboxItemResponseDto, userId: string): ContactsNotificationRecord {
  return {
    id: row.recipientId,
...
interface ContactsNotificationsState {
  items: ContactsNotificationRecord[];
  isLoading: boolean;
...
export const useContactsNotificationsStore = create<ContactsNotificationsState>()((set, get) => ({
  items: [],
  isLoading: false,
  error: null,
  unreadTotal: 0,
...
export function selectContactsInbox(items: ContactsNotificationRecord[], userId: string) {
  return items
    .filter((x) => x.recipientUserId === userId)
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

export function countUnreadContactsInbox(items: ContactsNotificationRecord[], userId: string) {
  return selectContactsInbox(items, userId).filter((x) => !x.readAt).length;
}
