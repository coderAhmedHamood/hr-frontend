...
export function InventoryNotificationBellPopover() {
  ...
  const unreadCountApi = useInventoryNotificationsStore((s) => s.unreadTotal);
  const inbox = React.useMemo(() => selectInventoryInbox(items, userId), [items, userId]);
  const unread = unreadCountApi > 0 ? unreadCountApi : countUnreadInventoryInbox(items, userId);
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent align="end" className="w-[min(100vw-2rem,420px)] overflow-hidden p-0" dir="rtl">
        <div className="flex items-center justify-between gap-2 border-b border-border px-3 py-2.5">
          <p className="text-sm font-semibold text-foreground">إشعارات المخازن</p>
          {canUpdate ? (
            <Button
              type="button"
              variant="ghost"
        ...
        <div className="max-h-[min(60vh,380px)] overflow-y-auto">
          {inbox.length === 0 ? (
            ...
            <ul className="divide-y divide-border/60">
              {inbox.map((n) => {
                const sourceLabel =
                  (n.sourceKind && INVENTORY_SOURCE_KIND_LABELS[n.sourceKind]) ?? 'مخازن';
                const severityLabel =
                      ...
                      <div className="flex min-w-0 items-start gap-2">
                        <span
                          className={cn(
                            ...
                            INVENTORY_SEVERITY_DOT_CLASS[n.severity] ?? 'bg-muted-foreground',
                          )}
                          aria-hidden
                        />
                        <p
