...
export function CustomerCreditNotesListPage() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return creditNotes.filter((cn) => {
      if (!term) return true;
      return (
        cn.name.toLowerCase().includes(term) ||
        cn.customerName.toLowerCase().includes(term) ||
        (cn.originalInvoiceName && cn.originalInvoiceName.toLowerCase().includes(term)) ||
  ...
  return (
    <div className="flex flex-col gap-4" dir="rtl">
      <SetPageTitle
        ...
        descriptionAr="إدارة إشعارات الدائن ومردودات المبيعات للعملاء"
        iconName="Receipt"
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-background p-2 border border-border/40 shadow-xs">
        ...
        <div className="flex items-center gap-2 order-1 md:order-2">
          <Button
            ...
            onClick={() => router.push('/accounting/customers/credit-notes/new')}
          >
            جديد
          </Button>
          <div className="flex items-center gap-1.5 me-1">
            <span className="text-lg font-bold text-foreground">إشعارات الدائن</span>
          ...
          <div className="relative w-full flex items-center">
            <div className="relative flex-1 flex items-center">
              <Input
                ...
                placeholder="...بحث برقم الإشعار أو العميل"
        ...
        <div className="flex items-center gap-2 order-2 md:order-1">
          ...
          <span className="text-xs text-muted-foreground font-mono tabular-nums px-1">
            {creditNotes.length} / 1-{filtered.length}
          </span>
        </div>
      </div>
            ...
            {filtered.map((cn) => {
              ...
              return (
                <tr
                  ...
                  onClick={(e) => handleRowClick(cn.id, e)}
                >
                  <td className="px-3 py-3">
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => toggleSelectOne(cn.id)}
                      className="rounded border-border text-primary focus:ring-primary h-4 w-4 cursor-pointer"
                    />
                  </td>
                  ...
                  <td className="px-4 py-3 font-mono text-foreground text-start" dir="ltr">
                    {cn.amountUntaxed.toLocaleString('ar-SA', { minimumFractionDigits: 2 })} {cn.currency}
                  </td>
                  <td className="px-4 py-3 font-mono font-bold text-foreground text-start" dir="ltr">
                    {cn.amountTotal.toLocaleString('ar-SA', { minimumFractionDigits: 2 })} {cn.currency}
                  </td>
                  <td className="px-4 py-3 font-mono font-semibold text-start" dir="ltr">
                    <span className={cn.amountDue > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-muted-foreground'}>
                      {cn.amountDue.toLocaleString('ar-SA', { minimumFractionDigits: 2 })} {cn.currency}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    {cn.paymentState === 'paid' && (
