...
export function CustomersListPage() {
  ...
  const [hasInvoiceFilter, setHasInvoiceFilter] = React.useState(true);
  ...
  return (
    <div className="flex flex-col gap-4">
      <SetPageTitle
        titleAr="العملاء"
        descriptionAr="إدارة العملاء والحسابات والفواتير"
        iconName="Users"
      />
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg bg-background p-2 border border-border/40 shadow-xs">
        ...
        <div className="flex items-center gap-2 order-1 md:order-2">
          ...
          <div className="flex items-center gap-1.5 me-1">
            <span className="text-lg font-bold text-foreground">العملاء</span>
            ...
            <div className="relative flex-1 flex items-center">
              ...
              {hasInvoiceFilter && (
                <div className="absolute end-7 top-1/2 -translate-y-1/2 flex items-center gap-1 rounded bg-primary/10 px-2 py-0.5 text-xs font-medium text-primary border border-primary/20">
                  ...
                  <button
                    type="button"
                    onClick={() => setHasInvoiceFilter(false)}
        ...
        <div className="flex items-center gap-2 order-2 md:order-1">
          ...
          <span className="text-xs text-muted-foreground font-mono tabular-nums px-1">
            {customers.length} / 1-{filtered.length}
          </span>
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-border/40">
            {filtered.map((cust) => {
              ...
              const initial = cust.name.trim().charAt(0) || 'ع';
              return (
                <tr
                  key={cust.id}
                  ...
                  onClick={(e) => handleRowClick(cust.id, e)}
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div
                        ...
                        style={{ backgroundColor: cust.avatarColor || '#9333ea' }}
                      >
                        {initial}
                      </div>
