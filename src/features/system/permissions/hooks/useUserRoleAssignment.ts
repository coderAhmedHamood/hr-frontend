...
export function useUsersForRole(roleId: string | null) {
  ...
  const usersQuery = useQuery({
    queryKey: PERMISSIONS_KEYS.usersList,
    queryFn: () => usersApi.getAll({ limit: 200, ...organizationActiveListStatusQuery() }),
    staleTime: 5 * 60 * 1000,
  });

  const assignedQuery = useQuery({
    queryKey: PERMISSIONS_KEYS.roleUsers(roleId),
    queryFn: async () => {
  ...
  function invalidate() {
    void queryClient.invalidateQueries({ queryKey: PERMISSIONS_KEYS.roleUsers(roleId) });
  }
