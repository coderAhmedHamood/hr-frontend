...
export type SettlementPdfMode = 'filled' | 'blank';

type Props = {
  open: boolean;
  employee: Employee;
  onCancel: () => void;
  onApply: (mode: SettlementPdfMode) => void;
};

export function EmployeeSettlementPdfPrepDialog({ open, employee, onCancel, onApply }: Props) {
  if (!open) return null;
    ...
    <Dialog open={open} onOpenChange={(v) => { if (!v) onCancel(); }}>
      <DialogContent className="max-w-md gap-0 overflow-hidden p-0 sm:rounded-xl">
        ...
        <div className="space-y-3 px-5 py-5 text-right">
          <p className="text-sm text-muted-foreground">اختر طريقة إنشاء المخالصة:</p>
          <Button
            type="button"
            variant="luxe"
          ...
          >
            <span className="flex w-full flex-col items-stretch gap-0.5 text-right">
              ...
              <span className="text-xs font-normal opacity-80">
                الاسم، الجنسية، رقم الأحوال، والشركة تُملأ تلقائياً
              </span>
            </span>
          </Button>
          <Button
