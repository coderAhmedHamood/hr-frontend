...
import { FileText, Loader2, LogOut, Plus, RefreshCw, Send } from 'lucide-react';
...
export function EmployeeResignationsPanel({ employee, onOpenPdfPrep }: Props) {
  ...
  const handleSendToEmployee = async (item: EmployeeResignationDto) => {
    setSendingId(item.id);
    try {
      const updated = await sendToEmployee(item.id);
      if (!updated || !companyId) return;
      await sendResignationToEmployeeNotification({
        companyId,
        employeeId: employee.id,
        resignationId: updated.id,
        resignationNumber: updated.resignationNumber,
        createdBy,
      });
      toast.success('تم إرسال إشعار التوقيع للموظف');
    } catch (err) {
      const { displayMessage } = handleApiError(err, 'employee-resignations.notify');
