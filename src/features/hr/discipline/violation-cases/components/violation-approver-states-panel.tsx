...
import {
  violationApprovalModeLabelAr,
  violationApproverStatusLabelAr,
...
export function ViolationApproverStatesPanel({
  states,
  compact = false,
  className,
}: {
    ...
    <div className={cn('space-y-2 rounded-lg border border-border bg-muted/15 p-3', className)}>
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        <span className="text-[10px] text-muted-foreground">
          {violationApprovalModeLabelAr(states.approvalMode)}
        </span>
      </div>
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex shrink-0 items-center gap-1.5">
                ...
                <span className={cn('rounded-full border px-2 py-0.5 text-[10px] font-medium', statusPillClass(a.status))}>
                  {violationApproverStatusLabelAr(a.status)}
                </span>
              </div>
            </div>
          </div>
