...
export function ProductMediaGallery({ images, fallbackAlt, activeDescription, className }: ProductMediaGalleryProps) {
  ...
  return (
    <div className={cn('flex flex-col gap-3', className)}>
      ...
      {slides.length > 1 ? (
        <div className="flex justify-center gap-2 md:justify-start" role="tablist">
          {slides.map((slide, slideIndex) => (
            <button
              ...
              className={cn(
                'h-14 w-14 shrink-0 overflow-hidden rounded-md border-2 bg-muted transition-colors',
